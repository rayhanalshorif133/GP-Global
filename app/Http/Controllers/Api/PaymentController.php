                ...
                if($status == 1){
                    ...
                    if($service->type == "on-demand"){
                        ...
                        // subscriber:start
                         ...
                         if (strpos($redirect_success_url, "?") !== false) {
                        ...
                        if($service->notification_url){
                            $notify_url = $service->notification_url . '?msisdn=' . $consent->msisdn . '&keyword=' . $service->keyword . '&acr=' . $consent->customer_reference . '&status=1&type=subs&reason=ok&op_time=' . date('Y-m-d H:i:s');
                            Http::get($notify_url);
                        }
                    ...
                    if(isset($responseData['requestError'])){
                        ...
                        if($service->notification_url){
                            ...
                            $notify_url = $service->notification_url . '?msisdn=' . $consent->msisdn . '&keyword=' . $service->keyword . '&acr=' . $consent->customer_reference . '&status=9&reason=' . $resion .'&op_time=' . date('Y-m-d H:i:s');
                            Http::get($notify_url);
                        }
                        // send sms
                        ...
                        if($messageId == 'POL1001' && $service->keyword != 'GAJAL' && $service->keyword != 'ESD' && $service->keyword != 'ESW'){
                            return redirect('https://gpglobal.b2mwap.com/api/subscription?keyword=GAJAL&msisdn='.$consent->msisdn);
                        }
                    }


                    if($service->keyword == 'ESD'){
                        //$url = "https://subscription-grameenphone.esportsme.net/subscription/?keyword=ESD&msisdn=8801323174104&acr=1jspKzXuo7ZHv391aAbfSkFZtOR0nkXv&type=subs&result=failed&op_time=2024-10-01%2012:42:45";
                        header("Location: $url");
                        exit;
                        //dd('test');
                        //dd($url);

...
class SubsAndUnsubsController extends Controller
{
    public function subscription(Request $request){
        ...
        // https://gpglobal.b2mwap.com/api/subscription?keyword=MRPA&msisdn=8801307345647
        // http://127.0.0.1:8000/api/subscription?keyword=SFN&msisdn=8801701677479&portal_link=somethings&redirect_url=somethings
    ...
    public function unsubscription(Request $request){
        ...
        try {
            ...
            if($service->notification_url){
                $notify_url = $service->notification_url . '?msisdn=' . $subscriber->msisdn . '&keyword=' . $service->keyword . '&acr=' . $acr_key . '&status=0&reason=ok&op_time=' . date('Y-m-d H:i:s');
                Http::get($notify_url);
            }

...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // SELECT count(*) as total FROM `consents` WHERE `created_at` LIKE '%2023-12-17%'


        // yesterday Consent hits: 403
// Consent back: 94
// Success: 63
        ...
        /* 
        SELECT DISTINCT `consents`.`msisdn`, `consent_back_logs`.`type` FROM `consents` INNER JOIN `consent_back_logs` ON `consent_back_logs`.`msisdn` = `consents`.`msisdn` WHERE `consents`.`service_id` = 1 AND `consents`.`created_at` LIKE '%2023-12-17%' AND `consent_back_logs`.`created_at` LIKE '%2023-12-17%' AND `consent_back_logs`.`type` = 'success'
        */ 
